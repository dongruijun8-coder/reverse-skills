...
- **Action:** Wait 2 seconds → retry same operation → call `state_phase_fail(project_dir, phase, error, retry=true)` → increment retry_count
- **User visibility:** None (silent)

## L2: Strategy Degradation
- **Trigger:** All attempts at current strategy layer exhausted (retry_count > 3)
  → `state_phase_fail` returns `status: EXIT` with `action: DEGRADE`
...
- **Action:** `state_save(project_dir, phase_status="PAUSED")` → generate structured pause report → wait for user input
- **User visibility:** Full pause report (5-15 lines)

## L5: App Abandonment
- **Trigger:** User replies "skip"/"abort" to L4 pause, OR 24h timeout
- **Action:** Save all intermediate artifacts → write case with result "abandoned" → `state_save(project_dir, phase_status="ABANDONED")` → mark resumable
